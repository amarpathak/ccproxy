...
This guide helps you diagnose and resolve common issues with ccproxy.
...
2. Refresh the token manually:
   - Remove and re-add the account:
     ```bash
     ccproxy --remove <account-name>
     ccproxy --add-account <account-name>
     ```
...
**Solutions**:
1. ccproxy automatically attempts to refresh expired tokens
...
ccproxy detects rate limits through response headers and HTTP status codes:
...
cat /tmp/ccproxy-logs/app.log | grep "rate limited"
...
1. ccproxy automatically rotates to the next available account
...
4. Pause/resume accounts as needed:
   ```bash
   ccproxy --pause <account-name>
   ccproxy --resume <account-name>
   ```
...
1. Check log file size (auto-rotates at 10MB):
   ```bash
   ls -lh /tmp/ccproxy-logs/app.log
   ```
...
- **macOS**: `~/.config/ccproxy/ccproxy.json`
- **Linux**: `~/.config/ccproxy/ccproxy.json`
- **Windows**: `%LOCALAPPDATA%\ccproxy\ccproxy.json` or `%APPDATA%\ccproxy\ccproxy.json`
...
1. Validate JSON syntax:
   ```bash
   cat ~/.config/ccproxy/ccproxy.json | jq .
   ```

2. Reset to defaults:
   ```bash
   # Backup current config
   cp ~/.config/ccproxy/ccproxy.json ~/.config/ccproxy/config.backup.json
   # Remove corrupted config
   rm ~/.config/ccproxy/ccproxy.json
...
1. Check database file permissions:
   ```bash
   # macOS/Linux
   ls -la ~/.config/ccproxy/ccproxy.db
   
   # Windows
   dir %LOCALAPPDATA%\ccproxy\ccproxy.db
   ```
...
2. Create the directory if it doesn't exist:
   ...
   # Windows
   mkdir %LOCALAPPDATA%\ccproxy
   ```

3. Use a custom database path:
   ```bash
   export ccproxy_DB_PATH=/path/to/custom/ccproxy.db
   bun start
   ```
...
3. Check for database corruption:
   ```bash
   sqlite3 ~/.config/ccproxy/ccproxy.db "PRAGMA integrity_check;"
   ```
...
3. Check logs for async writer errors:
   ```bash
   grep "async-db-writer" /tmp/ccproxy-logs/app.log
   ```
...
**Solutions**:
1. Ensure only one instance of ccproxy is running:
   ```bash
   ps aux | grep "bun start" | grep -v grep
   ps aux | grep "ccproxy --serve" | grep -v grep
   ```

2. Kill any zombie processes:
   ```bash
   pkill -f "bun start"
   pkill -f "ccproxy --serve"
   ```

3. Check for hanging database connections:
   ```bash
   lsof ~/.config/ccproxy/ccproxy.db
   ```
...
3. Check if streaming is working:
   ...
   grep "Streaming response" /tmp/ccproxy-logs/app.log
   ```
...
1. Check if requests are being recorded:
   ...
   sqlite3 ~/.config/ccproxy/ccproxy.db "SELECT COUNT(*) FROM requests WHERE timestamp > strftime('%s', 'now', '-1 hour') * 1000;"
   ```
...
2. Check for usage extraction errors:
   ```bash
   grep "extractUsageInfo" /tmp/ccproxy-logs/app.log
   ```

3. Verify model pricing data:
   ```bash
   # Pricing updates every 24 hours by default
   grep "Fetching latest pricing" /tmp/ccproxy-logs/app.log
   ```
...
- **All platforms**: `/tmp/ccproxy-logs/app.log`
- **Windows**: `%TEMP%\ccproxy-logs\app.log`
...
```bash
# View real-time logs
tail -f /tmp/ccproxy-logs/app.log
```
...
```bash
# View only errors
grep "ERROR" /tmp/ccproxy-logs/app.log

# View warnings and errors
grep -E "WARN|ERROR" /tmp/ccproxy-logs/app.log
```

**Filter by component**:
```bash
# View only proxy logs
grep "\[Proxy\]" /tmp/ccproxy-logs/app.log

# View only server logs
grep "\[Server\]" /tmp/ccproxy-logs/app.log
```
...
- Check account status: `ccproxy --list`
...
**Solutions**:
1. Ensure only one ccproxy instance is running
2. Kill any zombie processes
...
3. Clear history if data is corrupted: `ccproxy --clear-history`
...
**Solutions**:
1. Validate JSON syntax: `cat ~/.config/ccproxy/ccproxy.json | jq .`
...
3. Use custom path: `export ccproxy_DB_PATH=/custom/path/db.db`
...
| Variable | Description | Default | Example |
|----------|-------------|---------|---------|
| `ccproxy_CONFIG_PATH` | Custom config file location | Platform-specific | `/opt/ccproxy/config.json` |
| `ccproxy_DB_PATH` | Custom database location | Platform-specific | `/opt/ccproxy/data.db` |

### Logging and Debugging

| Variable | Description | Default | Example |
|----------|-------------|---------|---------|
| `ccproxy_DEBUG` | Enable debug mode | `0` | `1` |
...
export ccproxy_CONFIG_PATH=/etc/ccproxy/config.json
export ccproxy_DB_PATH=/var/lib/ccproxy/data.db
export PORT=3000
bun start
...
### Q: How do I know if ccproxy is working?
...
### Q: Can I use ccproxy with multiple client applications?

**A**: Yes, ccproxy acts as a transparent proxy. Point any Claude API client to `http://localhost:8080` instead of `https://api.anthropic.com`.
...
- **Windows**: `%LOCALAPPDATA%\ccproxy\ccproxy.db` or `%APPDATA%\ccproxy\ccproxy.db`
...
1. Database file (`ccproxy.db`)
2. Config file (`ccproxy.json`)
...
4. Check if requests are being recorded: `sqlite3 ~/.config/ccproxy/ccproxy.db "SELECT COUNT(*) FROM requests;"`
...
### Q: Can I use ccproxy in production?
...
### Q: What's the difference between running with bun start vs ccproxy --serve?

**A**: Both commands start the server:
- `bun start`: Runs the server using the npm script
- `ccproxy --serve`: Runs the server directly via the CLI binary
...
2. **Error Logs**:
   ...
   tail -n 100 /tmp/ccproxy-logs/app.log
   ```

3. **Configuration** (sanitized):
   ```bash
   # Remove sensitive data before sharing
   cat ~/.config/ccproxy/ccproxy.json | jq 'del(.client_id)'
   ```
...
env | grep -E "ccproxy|CLIENT_ID|PORT|LB_STRATEGY|LOG_|PROXY" | sort
echo ""

echo "=== Process Info ==="
ps aux | grep -E "bun start|ccproxy" | grep -v grep
echo ""
...
echo "=== Database Info ==="
if [ -f "$HOME/.config/ccproxy/ccproxy.db" ]; then
    echo "Database size: $(du -h "$HOME/.config/ccproxy/ccproxy.db" | cut -f1)"
    echo "Request count: $(sqlite3 "$HOME/.config/ccproxy/ccproxy.db" "SELECT COUNT(*) FROM requests;" 2>/dev/null || echo "Could not query")"
    echo "Account count: $(sqlite3 "$HOME/.config/ccproxy/ccproxy.db" "SELECT COUNT(*) FROM accounts;" 2>/dev/null || echo "Could not query")"
else
...
if [ -f "/tmp/ccproxy-logs/app.log" ]; then
    grep "ERROR" /tmp/ccproxy-logs/app.log | tail -20
else
...
if [ -f "/tmp/ccproxy-logs/app.log" ]; then
    grep -E "rate.?limit" /tmp/ccproxy-logs/app.log | tail -10
else
    echo "Log file not found"
fi
echo ""
...
echo "=== Account Status ==="
ccproxy --list 2>/dev/null || echo "Could not get account list"
echo ""
...
tail -f /tmp/ccproxy-logs/app.log | grep -E "INFO|WARN|ERROR"
```
...
4. **Rate Limits**
   ```bash
   grep "rate.?limit" /tmp/ccproxy-logs/app.log | tail -10
   ```
...
| Problem | Quick Fix |
|---------|-----------|
| All accounts rate limited | Add more accounts: `ccproxy --add-account newaccount` |
| Token expired | Re-authenticate: `ccproxy --remove account && ccproxy --add-account account` |
| Database locked | Kill duplicate processes: `pkill -f "bun start"` |
| Port in use | Use different port: `PORT=3000 bun start` |
| Config corrupted | Reset config: `rm ~/.config/ccproxy/ccproxy.json` |
| Analytics missing | Clear history: `ccproxy --clear-history` |
| Slow responses | Check session duration settings (default 1 hour) |

Remember: Most issues can be resolved by checking logs, verifying account status, and ensuring proper network connectivity. When in doubt, restart the service with debug logging enabled: `ccproxy_DEBUG=1 LOG_LEVEL=DEBUG bun start`