...
This document outlines the security considerations, practices, and recommendations for the ccproxy load balancer system.

## ⚠️ Critical Security Notice

**IMPORTANT**: ccproxy is designed for local development and trusted environments. The current implementation has several security limitations:
...
ccproxy is a load balancer proxy that manages multiple OAuth accounts to distribute requests to the Claude API. The system handles sensitive authentication tokens and request/response data, requiring careful security considerations.
...
#### 2. Key Management
- Use environment variable for encryption key: `ccproxy_ENCRYPTION_KEY`
...
**Risk**: Running ccproxy with default settings exposes it to the network
...
ccproxy_DEBUG=0            # Set to 1 only for debugging

# Configuration
ccproxy_CONFIG_PATH=/path/to/config.json  # Custom config location
...
sqlite3 ccproxy.db "SELECT COUNT(*) as count, account_used 
FROM requests 
...
sqlite3 ccproxy.db "SELECT * FROM audit_log WHERE action LIKE '%config%'"
```
...
### Key Takeaways
1. **ccproxy prioritizes functionality over security** - suitable for development, not production
