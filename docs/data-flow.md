# ccproxy Data Flow Documentation

## Overview

ccproxy is a load balancer proxy for Claude API that distributes requests across multiple OAuth accounts to avoid rate limiting. This document details the complete data flow through the system, including request lifecycle, error handling, token refresh, rate limit management, and streaming response capture.
...
ccproxy uses a modular architecture with the following key components:
...
The request lifecycle in ccproxy follows these main stages:

1. **Request Reception**: Client sends request to ccproxy server
...
sequenceDiagram
    participant Client
    participant Server as ccproxy Server
...
The ccproxy data flow is designed to:
