...
This guide covers all configuration options for ccproxy, including file-based configuration, environment variables, and runtime API updates.
...
ccproxy uses a flexible configuration system that supports:
...
Configuration is managed through the `@ccproxy/config` package, which provides automatic loading, validation, and change notifications.
...
2. **Configuration file** - Values from `~/.config/ccproxy/ccproxy.json` (or custom path)
...
- **Linux/macOS**: `~/.config/ccproxy/ccproxy.json` (or `$XDG_CONFIG_HOME/ccproxy/ccproxy.json`)
- **Windows**: `%LOCALAPPDATA%\ccproxy\ccproxy.json` (or `%APPDATA%\ccproxy\ccproxy.json`)
- **Custom path**: Set via `ccproxy_CONFIG_PATH` environment variable
...
| `ccproxy_DEBUG` | string | - | Set to `"1"` to enable debug mode with console output |
...
| `ccproxy_CONFIG_PATH` | - | string | `ccproxy_CONFIG_PATH=/etc/ccproxy.json` |
...
| `ccproxy_DEBUG` | Enable debug mode with console output | - | `ccproxy_DEBUG=1` |
| `ccproxy_DB_PATH` | Custom database file path | Platform-specific | `ccproxy_DB_PATH=/var/lib/ccproxy/db.sqlite` |
...
⚠️ **NOTE**: Only the `"session"` strategy is available in ccproxy. Other strategies (round-robin, least-requests, weighted) have been removed from the codebase as they can trigger Claude's anti-abuse systems and result in account bans.
...
ccproxy performs validation on:
...
2. Export current configuration:
   ```bash
   curl http://localhost:8080/api/config > ~/.config/ccproxy/ccproxy.json
   ```

3. Edit and format the file:
   ```bash
   jq '.' ~/.config/ccproxy/ccproxy.json > temp.json && mv temp.json ~/.config/ccproxy/ccproxy.json
   ```
...
1. **Configuration location**: Move from `~/.ccproxy/config.json` to platform-specific paths
...
```bash
cp ~/.config/ccproxy/ccproxy.json ~/.config/ccproxy/ccproxy.json.backup
```
...
1. **Configuration not loading**:
   - Check file permissions: `ls -la ~/.config/ccproxy/`
   - Verify JSON syntax: `jq '.' ~/.config/ccproxy/ccproxy.json`
